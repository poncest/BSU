...
# * FIX 1 ----
#   - readxl::read_excel*() now auto-names missing columns `...1`, `...2`, instead of X__1, X__2
...
definitions_tbl <- definitions_raw_tbl %>%
    ...
    filter(!is.na(B)) %>%
    separate(B, into = c("key", "value"), sep = " '", remove = TRUE) %>%
    rename(column_name = A) %>%
    mutate(key = as.numeric(key)) %>%
    mutate(value = value %>% str_replace(pattern = "'", replacement = ""))
definitions_tbl
# * END OPTIONAL SOLUTION 2 ----
